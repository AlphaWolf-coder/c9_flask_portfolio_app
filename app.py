...
@app.route('/<name>')
def profile(name):
	name_entry = name + " is a Game GEEK!!"
	return render_template('index.html', name=name_entry)
...
@app.route('/factorial_numbers', methods=['GET','POST'])
def factorial_numbers_post():
	  # --> ['5', '6', '8']
	  # print(type(request.form['text']))
	  if request.method == 'GET':
	  	return render_template('factorial_numbers.html')
	  elif request.method == 'POST':
  	      print(request.form['text'].split())
  	      total = 1
  	      try:
#   	      	fact = request.form['text']:
# 		while fact >= 1:
